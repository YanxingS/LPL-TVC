...
bool DAC_CONNECTION::update() {
    if (this->client.available()) {
        // Read available data into message buffer
        while (this->client.available()) {
            this->message[this->message_length] = this->client.read();
            this->message_length++;
        }

        // Reset message length for next read
        this->message_length = 0;

        // Determine state based on received command
        if (strcmp(this->message, "VECTR") == 0) {
            // START TVC
            state = VECTOR;
        ...
        else if (strcmp(this->message, "CALBR") == 0) {
            // CALIBRATE
            state = CALIBRATE;
        ...
        else if (strcmp(this->message, "ABORT") == 0) {
            // ABORT
            state = ABORT;
        } 
        else {
